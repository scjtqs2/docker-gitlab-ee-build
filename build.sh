...
for LATEST in "${no_match_tags[@]}"; do
    ...
    echo "DOWNLOAD_URL_amd64=https://packages.gitlab.com/gitlab/gitlab-ee/packages/ubuntu/focal/gitlab-ee_${LATEST}_amd64.deb/download.deb" >> RELEASE
...
#    docker run --privileged --rm tonistiigi/binfmt --install all
    docker buildx  build --platform  linux/arm64  -t ${DOCKER_NAME}/gitlab-ee-arm64:${LATEST} --build-arg TARGETARCH=arm64 -f Dockerfile --push . || exit 1
    cd ../../
    git add latest
    git add version
